	...
	B: PacketBytes,
{
	...
	pub(crate) async fn send(
		...
		match kind {
			Kind::Request => {
				...
				let sr = self
					.msg_to_server
					.send(Message::Request(packet, ResponseSender::new(tx)))
					.await;
	...
	pub(crate) fn packet_error(
		...
		match kind {
			...
			Kind::Close | Kind::Ping | Kind::StreamClosed => {
				Err(TaskError::Packet(e))
			}
			k => Err(TaskError::WrongPacketKind(k.to_str())),
		}
	}
